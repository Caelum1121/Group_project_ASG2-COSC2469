public class OptimizedGuesser { //findLengthBinary, trySingleCharacterPatterns, Smarter Initial Guessing, Heuristic Search, Intelligent Pruning, Dynamic Strategy Selection,
    ...
    /**
     * Try single character patterns (e.g., "BBBBBB", "AAAAAA", etc.)
     * This optimizes for cases where the secret code is one character repeated
     */
    private String trySingleCharacterPatterns(SecretCode code, int length) {
        ...
        for (char c : CHARACTERS) {
            ...
            if (score == length) {
                System.out.println("Found single character pattern: " + candidate);
                return candidate;
            }
        }

        System.out.println("Single character patterns failed, proceeding with complex strategy...");
        return null;
    }

    private String solveWithDynamicStrategy(SecretCode code, int length) {
        // First, try single character patterns (optimized for cases like "UUUUUUUUUU")
        String singleCharResult = trySingleCharacterPatterns(code, length);
        if (singleCharResult != null) {
            return singleCharResult;
        }

        // If single character patterns don't work, use original strategy
        if (length <= 6) {
